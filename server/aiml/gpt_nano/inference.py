...
def remove_after_string(text, target_string):
    # Find the position of the target string
    position = text.find(target_string)
    if position != -1:
        # Return the text up to (but not including) the target string
        return text[:position]
    # If the target string is not found, return the original text
    return text
...
if __name__ == "__main__":
    ...
    response = compare_gpt_models("Once upon a time", 50)
    end_time = time.time()
    print(response)
