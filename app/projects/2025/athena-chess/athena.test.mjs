 ...
 *
 *   node --experimental-strip-types app/projects/2025/athena-chess/athena.test.mjs
 *
...
const modelsDir = path.join(
  repo,
  "public/assets/projects/2025/athena-chess/models",
);

const meta = JSON.parse(
  readFileSync(path.join(modelsDir, "meta.json"), "utf8"),
);
const fixtures = JSON.parse(
  readFileSync(
    process.argv[2] ?? path.join(here, "athena.fixtures.json"),
    "utf8",
  ),
);
...
for (const c of fixtures.cases) {
  ...
  assert.strictEqual(
    moveIndex.get(c.uci),
    c.action_idx,
    `action_idx mismatch for ${c.uci}`,
  );
}
console.log(
  `✓ tokenizer + action index match on ${fixtures.cases.length} fixtures`,
);
...
console.log(
  `✓ onnx logits match PyTorch (max abs err ${maxErr.toExponential(2)}), argmax bins match`,
);
...
for (let b = 0; b < bins; b++) {
  assert.ok(
    Math.abs(single[b] - logits[3 * bins + b]) < 1e-4,
    "batch size changed the output",
  );
}
...
for (let ply = 0; ply < 24 && !game.isGameOver(); ply++) {
  ...
  for (const m of legal)
    assert.ok(moveIndex.has(m.uci), `${m.uci} missing from action vocab`);
...
for (const m of ontoA8)
  assert.ok(moveIndex.has(m.lan), `${m.lan} missing from action vocab`);
...
assert.strictEqual(
  promotionRanked[0].uci,
  "a7a8q",
  "expected the queen promotion to rank first",
);
console.log("✓ promotions are encodable and ranked (picks a8=Q)");

// --- 5. Decoder round trip ----------------------------------------------------
const { K, M, output_bins } = meta.output_encoder;
assert.deepStrictEqual(decodeBin(output_bins - 1, meta), {
  winProb: 1,
  mate: 0,
});
assert.deepStrictEqual(decodeBin(output_bins - 2, meta), {
  winProb: 1,
  mate: 1,
});
